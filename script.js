...
function load_step(step_to_load) {
	document.getElementById('content' + step_to_load).style.transition = '0.8s';
	document.getElementById('content' + step_to_load).style.opacity = '0';
	document.getElementById('content' + step_to_load).style.visibility =
		'hidden';
	document.getElementById('content' + step_to_load).style.opacity = '0';
	document.getElementById('content' + (step_to_load + 1)).style.transition =
		'0s';
	document.getElementById('content' + (step_to_load + 1)).style.visibility =
		'visible';
	document.getElementById('content' + (step_to_load + 1)).style.transition =
		'1.6s';
	document.getElementById('content' + (step_to_load + 1)).style.opacity = '1';
...
function check_password(step_to_check) {
	if (
		document.getElementById(
			'input' + (steps_on_password.indexOf(step_to_check) + 1)
		...
		document.getElementById(
			'input' + (steps_on_password.indexOf(step_to_check) + 1) + '_return'
	...
	for (var i = 0; i < document.body.clientWidth / 50; i++) {
		for (var j = 0; j < document.body.clientHeight / 65; j++) {
			...
			background_div =
				...
				(i * 50 + Math.floor(Math.random() * (15 - -15 + 1)) + -15) +
				'px;top:' +
				(j * 65 + Math.floor(Math.random() * (15 - -15 + 1)) + -15) +
				'px;//transform: rotate(' +
...
function background_animation() {
	window.addEventListener('mousemove', function (e) {
		for (var i = 0; i < id_index + 1; i++) {
			var element_x = parseInt(
				document.getElementById('l_' + i).style.left
			);

			var element_y = parseInt(
				document.getElementById('l_' + i).style.top
			);
			...
			if (
				...
				document
					.getElementById('l_' + i)
					.querySelector('path').style.fill = '#1e361e';

				document.getElementById('l_' + i).style.transform =
					'translateX( ' +
					(-100 +
						(e.clientX - element_x) +
						Math.abs(element_y - e.clientY)) +
					'px )    rotate( ' +
					(Math.floor(Math.random() * (15 - -15 + 1)) + -15) +
					'deg )';
			} else if (
				...
				document
					.getElementById('l_' + i)
					.querySelector('path').style.fill = 'none';
